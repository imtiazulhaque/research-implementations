...
def AttackHeatingControl(act_data_file, data_file, num_zones, co2_fresh_air, temp_fresh_air, rh_mixed, rh_supply):
    ################## constants ##################################
    ...
    act_zone_info = pd.read_csv(act_data_file)
    act_zone_occupant = [act_zone_info['Zone ' + str(i+1) + ' People (person)'].tolist()[0] for i in range(num_zones)]
    ...
    for i in range(num_zones):
        ...
        ############### v_temp_air ###############################
        s.add((v_temp_air[i] * CP_AIR * 25 ) / (0.89 * 2118.87) == ( zone_equip_load[i] + zone_occupant[i] * zone_pp_load[i]))
        ...
        s.add((v_mixed_air[i] * CP_AIR * (temp_supply_air[i] - zone_temp_setpoint[i]) ) / (0.89 * 2118.87) == ( zone_equip_load[i] + zone_occupant[i] * zone_pp_load[i]))
        ...
        ############# attack #######################################
        s.add(act_zone_occupant[i] * ((zone_pp_co2[i] * 1000000 * SAMPLING_TIME ) / zone_volume[i]) == 
              (att_zone_co2[i] - (( 1 - ( v_mixed_air[i] * SAMPLING_TIME ) / zone_volume[i]) * zone_co2_setpoint[i] + 
                                     ( v_mixed_air[i] * SAMPLING_TIME * co2_mixed_air[i]) / zone_volume[i])))
        
        s.add((v_mixed_air[i] * CP_AIR * (temp_supply_air[i] - att_zone_temp[i]) ) / (0.89 * 2118.87) == ( zone_equip_load[i] + act_zone_occupant[i] * zone_pp_load[i]))
        
        
        ############### temperature control algorithm ############
        s.add(Implies(v_vent_air[i] >= v_temp_air[i] , v_return_air[i] == 0))
        s.add(Implies(v_vent_air[i] < v_temp_air[i] ,  temp_supply_air[i] == 49))
        
        ############### other constraints ########################
        s.add(v_return_air[i] >= 0)
        s.add(temp_supply_air[i] <= 49)
    ...
    for i in range(num_zones):
        ...
        #print("v mixed air",i, v_mixed_air[i])
        #print("temp mixed air", temp_mixed_air[i])
        #print("temp supply air", temp_supply_air[i])
        
        #print("v return air", v_return_air[i])
        #print("v fresh air", v_fresh_air[i])
        #print("att zone co2", i+1, float(Fraction(str(s.model()[att_zone_co2[i]]))))
        #print("att zone temp", i+1, float(Fraction(str(s.model()[att_zone_temp[i]]))))
        
        #print('temp_mixed',temp_mixed, 'temp_supply', temp_supply, 'v_mixed_air', v_mixed_air)
        #cost = HVACCost.HVACCost(temp_mixed, rh_mixed, temp_supply, rh_supply, m_coil, temp_coil, v_mixed_air[i] / 2118.87, "kWh")
        cost = HeatingCostCalculation.HeatingCostCalculation(temp_mixed, rh_mixed, temp_supply, rh_supply, m_coil, temp_coil, v_mixed_air[i] / 2118.87, "kWh")
        total_cost += cost
    return total_cost